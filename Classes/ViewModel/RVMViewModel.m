...
@synthesize didLoadingSignal = _didLoadingSignal;
...
- (RACSignal *)didLoadingSignal {
    if (_didLoadingSignal == nil) {
        @weakify(self);
        
        _didLoadingSignal = [ [[RACObserve(self, loading) filter:^BOOL(NSNumber * value) {
            return  value.boolValue;
        }] map:^id _Nullable(id  _Nullable value) {
            @strongify(self);
            return self;
        }] setNameWithFormat:@"%@ -didLoadingSignal",self ];
    }
    return _didLoadingSignal;
}
